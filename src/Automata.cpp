...
void Automata::coin(int amount) 
{
    if (state == STATES::WAIT) 
    {
        if (amount > 0) 
        {
            cash += amount;
            state = STATES::ACCEPT;
            std::cout << "Introduced " << amount << " $. Balance: " << cash << " $. State: ACCEPT.\n";
        }
        else
        {
    ...
    {
        std::cout << "It is not possible to deposit money now (condition " << static_cast<int>(state) << ").\n";
    }
}
...
std::string Automata::getMenu() 
{
    ...
    {
        result += std::to_string(i + 1) + ". " + menu[i] + " - " + std::to_string(prices[i]) + " $.\n";
    }
    return result;
}
...
bool Automata::choice(int index) 
{
    ...
    state = STATES::CHECK;
    std::cout << "Selected " << menu[idx] << " (price " << selectedPrice << " $.). Switching to the CHECK state.\n";
    return true;
}
...
bool Automata::check() 
{
    ...
    {
        std::cout << "There's enough money (" << cash << " $. >= " << selectedPrice << " $.).\n";
        return true;
    }
    else 
    {
        std::cout << "There's not enough money: need " << selectedPrice << " $., Inserted " << cash << " $.\n";
        return false;
    }
}
...
void Automata::cook() 
{
    ...
    if (!check()) 
    {
        std::cout << "Insufficient funds. Deposit the money first or cancel the operation.\n";
        return;
    }
    cash -= selectedPrice;
    std::cout << "Written off " << selectedPrice << " $. Account balance: " << cash << " $.\n";
    state = STATES::COOK;
